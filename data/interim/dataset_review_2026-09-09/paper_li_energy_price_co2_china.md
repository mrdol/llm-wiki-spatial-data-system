...
benchmark_readiness:
  ...
  reason: "CORRECTION 2026-09-09 : formule complete deja resolue et verbatim (eqs. 3/5-6 du TEI) sur les 450 lignes du panel complet (30 provinces x 15 annees), sur decision explicite de l'utilisateur de ne PAS decouper en coupes annuelles ce jeu de petite taille (450 lignes, pas 'lourd', contrairement a gwqlasso/coree). Routes publiees sar_lag/sar_error (mappe vers sem_error) eligibles directement en base published_model -- W provinciale identique reappliquee par annee (panel spatial equilibre, pas une duplication). CV manuelle recommandee : grouper par province, respecter la chronologie (annee)."
  benchmark_task: "regression_continuous_panel"
```

- Decision: ready
- Manque principal: aucun blocage automatique detecte
- Raison: Formule complete deja resolue sur le panel entier (450 lignes) ; estimateurs generiques eligibles immediatement, SAR-lag/SAR-error verbatim du papier flagues en attente d'un support panel-spatial dedie (chantier separe, pas un blocage de promotion).
...
estimator_eligibility:
  eligible_estimators:
    - estimator: sar_lag
      ...
      source_ref: "Equation 5 (verbatim, TEI) : ln(CO2)_it = alpha_i + gamma*ln(EP)_it + beta*Control_it + rho*W*ln(CO2)_it + eta_t + xi_t + epsilon_t."
      notes: "Modele SAR-lag panel exact du papier. W provinciale identique reappliquee par annee (panel spatial equilibre, geometrie repetee legitime -- pas une duplication)."
    - estimator: sar_error
      basis: published_model
      source_ref: "Equation 6 (verbatim, TEI) : ln(CO2)_it = alpha_i + gamma*ln(EP)_it + beta*Control_it + lambda*W*upsilon_it + eta_t + xi_t + epsilon_t. Nom source du papier (SAR-error) -- mappe automatiquement vers l'estimateur package sem_error."
      notes: "Modele SAR-error panel exact du papier -- meme W que sar_lag."
  conditionally_eligible_estimators: []
  ineligible_reason: "n/a -- routes publiees (sar_lag/sem_error) eligibles directement, basis published_model (voir eligible_estimators)."
