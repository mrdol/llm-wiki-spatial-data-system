...
def dryad_get_href(session: requests.Session, href: str | None) -> dict[str, Any] | None:
    """Suit un lien API Dryad `_links.href`, qu'il soit relatif ou absolu."""

    if not href:
        return None
    url = href if href.startswith("http") else f"{DRYAD_HOST}{href}"
    try:
...
def _link_href(value: dict[str, Any], rel: str) -> str | None:
    """Lit une URL relationnelle Dryad dans un bloc `_links`."""

    links = value.get("_links") if isinstance(value.get("_links"), dict) else {}
    link = links.get(rel) if isinstance(links.get(rel), dict) else None
    href = link.get("href") if isinstance(link, dict) else None
    return str(href) if href else None


def fetch_dryad_detail(session: requests.Session, hit: dict[str, Any]) -> dict[str, Any]:
    """Charge la fiche detaillee et les fichiers individuels d'un dataset Dryad."""

    identifier = hit.get("identifier") or hit.get("doi") or hit.get("global_id") or hit.get("id")
    if not identifier:
        return hit

    detail = dryad_get(session, f"/datasets/{quote(str(identifier), safe='')}", {}) or hit
    version = dryad_get_href(session, _link_href(detail, "stash:version")) or {}
    files_payload = dryad_get_href(session, _link_href(version, "stash:files")) or {}
    ...
    for page in range(1, max_pages + 1):
        ...
        for hit in hits:
            if not isinstance(hit, dict):
                continue
            records.append(fetch_dryad_detail(session, hit))
    return records
