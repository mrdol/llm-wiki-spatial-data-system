...
build_specs <- function(y, x, coords = c("coord_x", "coord_y"),
  ...
  list(
    ...
    # SAR/SEM/SDM natifs spatialreg. Ils passent maintenant par un wrapper
    # parsnip custom spatialreg_reg(), donc par workflow() comme les autres
    # specifications. Le wrapper construit listw au fit et au predict.
    sar_lag = list(
      spec = spatialreg_reg(coords = coords, model_type = "SAR", k_neighbors = 8) |>
        parsnip::set_engine("spatialreg") |> parsnip::set_mode("regression"),
      formula = build_estimator_formula(y, c(x, coords))
    ),
    sem_error = list(
      spec = spatialreg_reg(coords = coords, model_type = "SEM", k_neighbors = 8) |>
        parsnip::set_engine("spatialreg") |> parsnip::set_mode("regression"),
      formula = build_estimator_formula(y, c(x, coords))
    ),
    sdm_mixed = list(
      spec = spatialreg_reg(coords = coords, model_type = "SDM", k_neighbors = 8) |>
        parsnip::set_engine("spatialreg") |> parsnip::set_mode("regression"),
      formula = build_estimator_formula(y, c(x, coords))
    ),
