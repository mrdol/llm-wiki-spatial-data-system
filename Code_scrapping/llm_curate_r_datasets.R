...
#
# Exemples :
# Rscript Code_scrapping/llm_curate_r_datasets.R --max_rows 5
# Rscript Code_scrapping/llm_curate_r_datasets.R --package ade4 --bundle macon
# Rscript Code_scrapping/llm_curate_r_datasets.R --category Bons_candidats_spatial --max_rows 20
...
build_curation_prompt <- function(catalog_row_json, inspection_text) {
...
curate_one_row <- function(row, dry_run = FALSE) {
  ...
  prompt <- build_curation_prompt(catalog_row_json, inspection_text)
...
main <- function() {
  ...
  if (args$dry_run) {
    result <- curate_one_row(catalog[1, , drop = FALSE], dry_run = TRUE)
    cat(result$prompt)
  ...
  for (i in seq_len(nrow(catalog))) {
    ...
    decision <- tryCatch({
      curate_one_row(row, dry_run = FALSE)
    }, error = function(err) {
      list(
        package = as.character(row$package),
        bundle = as.character(row$bundle),
