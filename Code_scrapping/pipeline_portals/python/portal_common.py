...
DEFAULT_PLAN_DIR = REPO_ROOT / "data" / "manifests" / "datasets"
DEFAULT_CANDIDATE_DIR = REPO_ROOT / "data" / "candidates"
DEFAULT_DATACANDIDATE_DOWNLOAD_DIR = REPO_ROOT / "data" / "candidates" / "datasets"
...
def clean_description_text(value: Any) -> str | None:
    """Return readable plain text from an API description or HTML fragment."""
    ...
    if not text.strip():
        return None
    cleaned = html_text(text)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    return cleaned or None


def description_excerpt(value: Any, *, max_chars: int = 1000) -> str | None:
    """Build a short, stable excerpt for fiches and catalog metadata."""

    cleaned = clean_description_text(value)
    if not cleaned:
        return None
    if len(cleaned) <= max_chars:
        return cleaned
    excerpt = cleaned[:max_chars].rsplit(" ", 1)[0].strip()
    return f"{excerpt}..."
...
def description_metadata(
    ...
    *,
    ...
    """Store full source description plus a short excerpt for downstream fiches."""

    cleaned = clean_description_text(value)
    return {
        "description": cleaned,
        "description_excerpt": description_excerpt(cleaned, max_chars=max_excerpt_chars),
        ...
        rows.append(
            {
                ...
                "description_excerpt": (record.get("description_metadata") or {}).get("description_excerpt"),
