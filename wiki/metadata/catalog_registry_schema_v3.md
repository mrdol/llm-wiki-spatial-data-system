...
> **Note (2026-06-30)** — This document describes the conceptual/JSON registry
> schema (field paths like `identity.title`, `content_metadata.variable_typology`).
> It is **not** a literal Markdown template. The actual `.md` fiche format
> enforced by `LLM-wiki-Assessment/eval/tier1_structural.py` for package-sourced
> datasets follows the **Bloc 1-6 structure** from `raw/docs_methodology/metadata_construction.pdf`
> (Bloc 1 — Formule et variables, Bloc 2 — Identification et DOI, Bloc 3 —
> Typologie des modeles, Bloc 4 — Typologie des donnees, Bloc 5 — Resolution
> et etendue, Bloc 6 — Reproductibilite, plus `## Related Pages`), generated by
> `Code_scrapping/r_catalog/generate_fiches.py`. Field labels inside each Bloc
> must stay literal/English (e.g. `Dataset name:`, `N observations:`) because
> Tier 1 matches them by regex anywhere in the body, independent of section
> headers — only `## Related Pages` requires an exact header match. See the
> `enrich-metadata` skill for the up-to-date generation flow. Older
> warehouse-sourced fiches (e.g. Dryad) predate this convention and use a
> different English-header layout (`## Identity`, `## License Metadata`, ...)
> that Tier 1 also still accepts — both layouts are valid as long as the same
