...
Format
     ...
     participation Factor. Did the individual participate in the labor
          force in 1975?  (This is essentially ‘wage > 0’ or ‘hours >
          0’.)
     ...
     repwage Wife's wage reported at the time of the 1976 interview
          (not the same as the 1975 estimated wage). To use the
          subsample with this wage, one needs to select 1975 workers
          with ‘participation == "yes"’, then select only those women
          with non-zero wage. Only 325 women work in 1975 and have a
          non-zero wage in 1976.
     ...
     fincome Family income, in 1975 dollars. (This variable is used to
          construct the property income variable.)

     tax Marginal tax rate facing the wife, and is taken from published
          federal tax tables (state and local income taxes are
          excluded). The taxable income on which this tax rate is
          calculated includes Social Security, if applicable to wife.

     meducation Wife's mother's educational attainment, in years.

     feducation Wife's father's educational attainment, in years.

     unemp Unemployment rate in county of residence, in percentage
          points.  (This is taken from bracketed ranges.)
...
References
     ...
     Altman, M., Gill, J., and McDonald, M.P.: _Numerical Issues in
     Statistical Computing for the Social Scientist_.  Hoboken, NJ:
     ...
     Wooldridge, J.M. (2002). _Econometric Analysis of Cross-Section
     and Panel Data_.  Cambridge, MA: MIT Press.
...
participation: factor ; missing=0 ; examples=yes

hours: integer ; missing=0 ; examples=1610, 1656, 1980

youngkids: integer ; missing=0 ; examples=1, 0

oldkids: integer ; missing=0 ; examples=0, 2, 3

age: integer ; missing=0 ; examples=32, 30, 35

education: integer ; missing=0 ; examples=12

wage: numeric ; missing=0 ; examples=3.354, 1.3889, 4.5455

repwage: numeric ; missing=0 ; examples=2.65, 4.04

hhours: integer ; missing=0 ; examples=2708, 2310, 3072

hage: integer ; missing=0 ; examples=34, 30, 40

heducation: integer ; missing=0 ; examples=12, 9

hwage: numeric ; missing=0 ; examples=4.0288, 8.4416, 3.5807

fincome: integer ; missing=0 ; examples=16310, 21800, 21040

tax: numeric ; missing=0 ; examples=0.7215, 0.6615, 0.6915

meducation: integer ; missing=0 ; examples=12, 7

feducation: integer ; missing=0 ; examples=7

unemp: numeric ; missing=0 ; examples=5, 11

city: factor ; missing=0 ; examples=no, yes

experience: integer ; missing=0 ; examples=14, 5, 15

college: factor ; missing=0 ; examples=no

hcollege: factor ; missing=0 ; examples=no
