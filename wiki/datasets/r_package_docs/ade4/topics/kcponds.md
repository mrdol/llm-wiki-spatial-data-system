...
Description
     ...
     This data set contains informations about 33 ponds in De Maten
     reserve (Genk, Belgium).
...
Format
     ...
     tab a data frame with 15 environmental variables (columns) on 33
          ponds (rows)
     ...
     nb the neighbourhood graph of the 33 sites (an object of class
          ‘nb’)
...
Details
     ...
     Variables of ‘kcponds$tab’ are the following ones : depth, area,
     O2 (oxygen concentration), cond (conductivity), pH, Fe (Fe
     concentration), secchi (Secchi disk depth), N (NNO concentration),
     TP (total phosphorus concentration), chla (chlorophyll-a
     concentration), EM (emergent macrophyte cover), FM (floating
     macrophyte cover), SM (submerged macrophyte cover), denMI (total
     density of macroinvertebrates), divMI (diversity
     macroinvertebrates)

Source

     Cottenie, K. (2002) Local and regional processes in a zooplankton
     metacommunity.  PhD, Katholieke Universiteit Leuven, Leuven,
     Belgium.
