...
|---|---|---|---|
| `Season` | `integer` | unknown | 0% |
| `total_live_mistletoe` | `integer` | unknown | 0% |
| `total_dead_mistletoe` | `integer` | unknown | 0% |
| `canopy_cover` | `integer` | unknown | 0% |
| `shrub_cover` | `integer` | unknown | 0% |
| `large_old_tree_total` | `integer` | unknown | 0% |
...
- License evidence: DataCite API record for DOI 10.5061/dryad.76hdr7sxp (checked 2026-08-18): rightsList = 'Creative Commons Zero v1.0 Universal'.
...
estimator_eligibility:
  ...
  ineligible_reason: "Mis a jour le 2026-09-18 : inla_spde_st (champ spatio-temporel espace x AR1, verifie sur les donnees reelles de paper_crane) resout desormais le premier des deux ecarts identifies -- la structure panel (T=5, Season) est geree nativement via son argument time=. Le second ecart reste bloquant pour une eligibilite pleine : le modele publie ajoute des effets aleatoires observateur/region (inla_spde_group) ET une structure temporelle (inla_spde_st) simultanement, or ces deux capacites existent separement dans le harnais mais ne sont pas combinees en une seule route de benchmark. Place en conditionally_eligible, pas en eligible_estimators."
