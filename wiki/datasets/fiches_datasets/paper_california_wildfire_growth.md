...
- Reference publication: Papier identifie via recherche web (session 2026-08-17), puis LU INTEGRALEMENT (PDF complet fourni par l'utilisateur, session 2026-09-23 -- aucun TEI n'existe pour ce papier, cherche par nom d'auteur et par DOI dans les 452 TEI et 422 PDF bruts, aucune correspondance ; les memoires universitaires ne sont pas couverts par le pipeline TEI actuel) : Hanley, H.S. (2022), 'Environmental Influences on Large Daily Wildfire Growth in California', Master's Thesis, San Jose State University, doi:10.31979/etd.5znn-tm8p (these avec DOI officiel, SJSU ScholarWorks). CONFIRME par lecture directe : le papier ajuste un modele Random Forest (grid search : max depth=15, min samples leaf=1, min samples split=5, n estimators=100 ; split train/test 70/30 chronologique) sur 16013 jours-incendie (10872 incendies individuels, 2003-2020, Californie frontiere Oregon a comte de Los Angeles) pour predire un SEUIL BINAIRE (croissance >10000 acres en 24h) -- AUC=0.831. Interpretation par SHAP + LOWESS : 100-hour dead fuel moisture <10%, temperature max >75F, humidite du sol (0-10cm) <12% sont les seuils de rupture identifies. AUCUNE matrice de poids spatiale, aucun terme SAR/GWR/spatial lag -- etude de feature importance ML pure, hors de portee du chantier W. formula_used CORRIGE le 2026-09-23 (etait T2+WS+mean_RH+ERC+BI+PDSI) : ces 6 variables sont celles de l'EXPERIENCE 3 du papier ('Index substitution for weather and fuel features', section 4.3 -- une variante de robustesse alternative), PAS le modele de reference. Le modele de reference (section 2.4 : 'This study's baseline random forest model was built using three variables for each side [of the fire behavior triangle]') utilise 9 variables precises : combustible (soil moisture=SMOIS, 100-hour dead fuel moisture=mean_wtd_moisture_100hr, chamise live fuel moisture=LFM_Chamise_New), meteo (max daily air temperature=T2_MAX -- pas T2, min daily relative humidity=min_RH -- pas mean_RH, mean daily wind speed=WS), topographie (slope=SLOPE, elevation=HGT, terrain ruggedness=VAR_SSO -- variable citee LITTERALEMENT dans le papier : 'the WRF variable, VAR_SSO (variance of subgrid-scale orography), was used for terrain ruggedness'). Les 9 verifiees disponibles localement, propres (7.5-7.7% NA de facon coherente entre elles -- donnees meteo WRF manquantes pour certains jours-incendie, pas un bug). Reserve : LFM_Chamise_New retenue plutot que LFM_Chamise_Old (le papier cite deux methodes, Nelson 2000=ancienne et Carlson et al. 2007=ML/nouvelle ; New est l'hypothese la plus probable pour une these de 2022, non confirmee a 100%). Final_size_perimeter (continue) reste un proxy documente du vrai seuil binaire du papier -- inchange par cette correction, deja accepte comme simplification. CSV original (Fire_03_20.csv) telecharge directement depuis Zenodo -- pas une reconstruction, N=23031 incendies avec coordonnees d'ignition reelles. Fichier drought_cumu_perc_area.csv (serie temporelle secheresse CA sans coordonnees) present dans le meme depot mais non utilise ici. package_include reste 'yes' (voir benchmark_readiness) : cette correction renforce la fidelite au modele publie, elle ne la degrade pas.
...
- formula_used: Final_size_perimeter ~ SMOIS + mean_wtd_moisture_100hr + LFM_Chamise_New + T2_MAX + min_RH + WS + SLOPE + HGT + VAR_SSO
...
- x_terms_used: SMOIS, mean_wtd_moisture_100hr, LFM_Chamise_New, T2_MAX, min_RH, WS, SLOPE, HGT, VAR_SSO
