...
- Reference publication: van Beeck Calkoen, S.T.S., Kuijper, D.P.J., Apollonio, M., Blondel, L., Dormann, C.F., Storch, I. & Heurich, M. (2023), Numerical top-down effects on red deer (Cervus elaphus) are mainly shaped by humans rather than large carnivores across Europe, Journal of Applied Ecology, doi:10.1111/1365-2664.14526. CSV telecharge directement depuis Dryad (10.5061/dryad.0cfxpnw7w, API OAuth) -- pas une reconstruction, N=534 sites d'etude identique au depot source (Data_SvBC_RedDeer.csv). README.md du depot documente exactement les variables : recherche litterature (annee, pays, zone d'etude, latitude, longitude, densite, chasse) + facteurs additionnels (productivite primaire nette, presence de grands carnivores, indice d'influence humaine, statut de protection, couverture forestiere, indice de secheresse de Palmer, indice de couverture neigeuse). MAJ preuve (session 2026-09-23) : texte integral desormais consulte (PDF en libre acces trouve hors TEI/corpus local, https://hogeveluwe.nl/media/ehddlkgf/van-beeck-calkoen-2023-numerical-top-down-effects-on-red-deer-mainly-shaped-by-humans-rather-than-lare-carnivores.pdf). Section 2.3 'Statistical analysis' confirme : modele GAM (mgcv::gam, Wood 2011), splines thin-plate (k=9) sur NPP/HII/couvert forestier/secheresse-PDSI/NDSI, carnivores et statut de protection en facteurs -- aucun terme spatial dans le modele estime. DHARMa (Hartig 2020) cite explicitement comme outil de diagnostic des residus ('Exploratory analyses and model diagnostics were performed using the DHARMa... package'), pas comme composante du modele. Echantillon analytique final confirme = 492 sites ('In total, data from 492 study sites across 28 European countries were included in the analyses') -- coherent avec le sous-ensemble deja isole dans [[paper_red_deer_topdown_492]], pas avec les 534 lignes brutes de cette fiche parent.
...
- Note N/T (session 2026-08-17, verification directe du `.rds`): "N observations" (534) est le nombre total de lignes du panel, pas le nombre d'unites spatiales distinctes. N spatial reel (geometries distinctes) = 526 ; panel NON EQUILIBRE (T par unite : min=1, mediane=1, max=3). Pour tout estimateur spatial explicite (SAR/GWR/BYM/CAR) necessitant une matrice de voisinage W, construire W sur les 526 unites spatiales distinctes, pas sur les 534 lignes du panel -- sinon des coordonnees dupliquees degenerent le calcul de voisinage/distance. Decision W (session 2026-09-23) : aucune matrice de voisinage construite. Le script R des auteurs (01. Model.R, deja lu en session anterieure) n'utilise DHARMa::testSpatialAutocorrelation() que comme diagnostic residuel (Moran's I a poids inverse-distance), pas comme terme spatial du modele estime -- ce diagnostic seul ne justifie pas une W de projet pour cette fiche, et aucun estimateur ici n'en consommerait une.
