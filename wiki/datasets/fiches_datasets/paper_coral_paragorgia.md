---
title: paper_coral_paragorgia
type: dataset
created: 2026-09-14
updated: 2026-09-17
sources:
  - data/final_datasets/sf/paper_coral_paragorgia.rds
...
- Topic: Donnees de paper-derived : paper_coral_paragorgia
...
- Geographic context: etendue sf: x [-179.9900055, 179.9888], y [-55.3814167, -25.228166]
...
- Local sf output: `data/final_datasets/sf/paper_coral_paragorgia.rds`
...
> Selection Y/X (paper-loader / curated evidence) : Pour `coral_paragorgia`, la ou les reponses `pa` viennent du loader papier et/ou des preuves de l article `Predicting the effects of climate change on deep-water coral distribution around New Zealand-Will there be suitable refuges for protection at the end of the 21st century?`. Les covariables X retenues sont `carbonate`, `mud`, `sand`, `bpi_fine`, `depth`, `slope_per`, `smtfinal`, `BEN_N_C`, `DETFLUX3_C`, `OM_CAL3_C`, `OXY_C`, `PBO_C`, `SO_C`, `SFR_OARG_C`. Les coordonnees (`lon`, `lat`), identifiants (les identifiants detectes), geometries et champs techniques sont exclus de X. Statut benchmark actuel : manual_review; la promotion package reste conditionnee au bloc benchmark_readiness.
...
| `SFR_OARG_C` | `numeric` | continuous | 0.5% |
...
- Dataset ID: `paper_coral_paragorgia`
...
- CRS EPSG: 4326
- CRS nom: WGS 84
- Spatial extent: x [-179.9900055, 179.9888], y [-55.3814167, -25.228166]
...
- License open: yes
- Reproducibility status: OK - loader R enregistre et reexecutable (`coral_paragorgia` dans build_sf_datasets_papers.R) ; source brute tracee dans inst/kg/paper_dataset_uses.json.
- Code available: yes (loader `coral_paragorgia` dans `code/r_catalog/build_sf_datasets_papers.R`)
...
- Reproducibility: OK - loader R enregistre et reexecutable (`coral_paragorgia` dans build_sf_datasets_papers.R) ; source brute tracee dans inst/kg/paper_dataset_uses.json.
