...
- formula_used: not_applicable - aucune covariable presente dans le jeu de donnees local ; le papier source utilise des indices de teledetection non inclus dans ce depot (cartographie de rendement seulement, yield ~ long*lat a but de visualisation, pas de regression).
...
modeling_evidence:
  ...
  equation_text: "not_applicable - aucune covariable presente dans le jeu de donnees local ; le papier source utilise des indices de teledetection non inclus dans ce depot (cartographie de rendement seulement, yield ~ long*lat a but de visualisation, pas de regression)."
  equation_family: regression
  model_family: "formule publication confirmee et utilisee"
  source_type: scientific_publication_or_package_documentation
  source_ref: "Kayad et al. (2016) PLOS ONE 11(6). NOTE : le papier regresse le rendement contre des indices satellites Landsat-8 (NIR, SAVI, NDVI, r jusqu'a 0.69) mais ces covariables ne sont PAS presentes dans l'objet R kayad.alfalfa (seulement harvest/lat/long/yield)."
  confidence: medium
```
