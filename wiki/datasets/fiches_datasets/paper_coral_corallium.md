---
title: paper_coral_corallium
type: dataset
created: 2026-09-14
updated: 2026-09-17
sources:
  - data/final_datasets/sf/paper_coral_corallium.rds
...
- Topic: Donnees de paper-derived : paper_coral_corallium
...
- Geographic context: etendue sf: x [-179.952334, 179.9975], y [-53.732, -24.665167]
...
- Local sf output: `data/final_datasets/sf/paper_coral_corallium.rds`
...
> Selection Y/X (paper-loader / curated evidence) : Pour `coral_corallium`, la ou les reponses `pa` viennent du loader papier et/ou des preuves de l article `Predicting the effects of climate change on deep-water coral distribution around New Zealand-Will there be suitable refuges for protection at the end of the 21st century?`. Les covariables X retenues sont `carbonate`, `mud`, `sand`, `bpi_fine`, `depth`, `slope_per`, `smtfinal`, `BEN_N_C`, `DETFLUX3_C`, `OM_CAL3_C`, `OXY_C`, `PBO_C`, `SO_C`, `SFR_OARG_C`. Les coordonnees (`lon`, `lat`), identifiants (les identifiants detectes), geometries et champs techniques sont exclus de X. Statut benchmark actuel : manual_review; la promotion package reste conditionnee au bloc benchmark_readiness.
...
| `SFR_OARG_C` | `numeric` | continuous | 1% |
...
- Dataset ID: `paper_coral_corallium`
...
- CRS EPSG: 4326
- CRS nom: WGS 84
- Spatial extent: x [-179.952334, 179.9975], y [-53.732, -24.665167]
- Time range: not applicable (cross-sectional dataset)
- CRS analyse recommande: pending — etendue continentale/mondiale (span=359.9deg) -- projection nationale non pertinente ; privilegier une projection equal-area continentale ou mondiale (ex: Albers equal-area continental, Behrmann/Mollweide pour une couverture mondiale)
...
- License open: yes
- Reproducibility status: OK - loader R enregistre et reexecutable (`coral_corallium` dans build_sf_datasets_papers.R) ; source brute tracee dans inst/kg/paper_dataset_uses.json.
- Code available: yes (loader `coral_corallium` dans `code/r_catalog/build_sf_datasets_papers.R`)
...
- Reproducibility: OK - loader R enregistre et reexecutable (`coral_corallium` dans build_sf_datasets_papers.R) ; source brute tracee dans inst/kg/paper_dataset_uses.json.
