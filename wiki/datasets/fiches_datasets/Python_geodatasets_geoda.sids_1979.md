...
Dataset spatial issu du package Python `geodatasets` (`sids_1979`).
...
- Candidate Y variables: `sids_rate`, `sids_deaths`
...
|---|---|---|---|---|
| `sids_rate` | `numeric` | continuous | [0, 6.1139] | 0% |
| `sids_deaths` | `numeric` | continuous | [0, 57] | 0% |


> Selection Y/X (claude-sonnet-4-6) : Le taux et le nombre de décès par mort subite du nourrisson (SIDS) sont les variables réponses naturelles de ce dataset épidémiologique. Le nombre de naissances totales, les naissances non-blanches et leur taux constituent des covariables explicatives classiques pour modéliser la variabilité spatiale du SIDS (exposition démographique et facteur socio-racial reconnu dans la littérature). NAME, FIPS et FIPSNO sont des identifiants/codes administratifs ignorés.
...
- Dataset ID: `Python_geodatasets_geoda.sids_1979`
- Dataset name: geodatasets::sids_1979
