...
Note complementaire (session 2026-09-23, revisee apres lecture integrale du PDF) : le papier parent (Song, Ahn, An & Jang 2021, Data in Brief) est un article 'data descriptor' qui ne publie pas lui-meme d'equation de regression -- deja documente dans la fiche parent [[paper_korea_hedonic_housing]] (session 2026-08-16). Une etude d'application du meme jeu a ete identifiee et LUE INTEGRALEMENT le 2026-09-23 (PDF complet fourni par l'utilisateur ; aucun TEI n'existe pour ce papier dans le corpus, cherche par DOI et par titre dans les 452 TEI et 422 PDF bruts, aucune correspondance) : Ahn, Jang & Song (2020), 'Economic impacts of being close to subway networks: A case study of Korean metropolitan areas', Research in Transportation Economics 83:100900, doi:10.1016/j.retrec.2020.100900. CONFIRME (pas juste rapporte par une discussion externe) : ce papier utilise un modele a lag spatial explicite (eq. 2-3) avec une matrice de poids entierement specifiee W_ij = 1/d_ij si d_ij < D (0 sinon), distance euclidienne entre logements, forme row-standardisee -- voir [[paper_korea_hedonic_housing]] > Bloc 1 pour le detail complet et les reserves (seuil D non chiffre dans le texte, effectifs par ville differents entre Ahn et al. 2015 et l'artefact local Zenodo). Ce lien reste au niveau du jeu PARENT complet (4 villes, toutes annees) -- aucune preuve locale ou externe qu'Ahn et al. estiment specifiquement le sous-echantillon pre1989 de ce decoupage temporel.
