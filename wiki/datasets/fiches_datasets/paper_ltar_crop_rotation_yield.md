...
- Note N/T (session 2026-08-17, verification directe du `.rds`): "N observations" (11970) est le nombre total de lignes du panel, pas le nombre d'unites spatiales distinctes. N spatial reel (geometries distinctes) = 11 ; panel NON EQUILIBRE (T par unite : min=228, mediane=704, max=2863). Pour tout estimateur spatial explicite (SAR/GWR/BYM/CAR) necessitant une matrice de voisinage W, construire W sur les 11 unites spatiales distinctes, pas sur les 11970 lignes du panel -- sinon des coordonnees dupliquees degenerent le calcul de voisinage/distance. Decision W (session 2026-09-23) : aucune matrice de voisinage construite. Le papier (Bowles et al. 2020, deja lu en session anterieure) modelise le rendement par un cadre bayesien hierarchique par site (RCI x temps), sans terme d'interaction spatiale entre les 11 sites ; avec seulement 11 unites, une W kNN=8 relierait 8 des 10 autres sites a chaque site (graphe quasi complet), non informatif.
...
- CRS analyse recommande: pending — multi-zones (span=26.2deg) -- etendue compatible avec un grand pays/une region ; verifier qu'une projection nationale/regionale existe et convient a cette zone avant de l'utiliser, sinon envisager une projection continentale equal-area
