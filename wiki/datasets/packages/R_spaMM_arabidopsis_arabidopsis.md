...
For 948 â€œaccessionsâ€ from European Arabidopsis thaliana populations, this data set merges the genotypic information at four single nucleotide polymorphisms (SNP) putatively involved in adaptation to climate (Fournier-Level et al, 2011, Table 1), with 13 climatic variables from Hancock et al. (2011).
...
> Note doc : response is binary so â€˜method="PQL/L"â€™ seems warranted (see

> Selection Y/X (claude-sonnet-4-6) : Les quatre SNPs binaires (pos*) sont les variables rÃ©ponses naturelles dans un contexte d'association gÃ©notype-environnement (landscape genomics), oÃ¹ l'on cherche Ã  expliquer la variation allÃ©lique par des pressions de sÃ©lection climatiques. Les 13 variables climatiques (tempÃ©rature, prÃ©cipitations, PAR, humiditÃ©, ariditÃ©, etc.) constituent les covariables explicatives; la colonne T est exclue car sa plage est identique Ã  dayLSp, suggÃ©rant une redondance ou une erreur de donnÃ©es.
